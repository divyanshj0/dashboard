// components/ConfigReportModal.js
'use client';
...
export default function ConfigReportModal({ onClose }) {
  ...
  const [selectedKeys, setSelectedKeys] = useState({}); // { deviceId: [key1, key2] }
  ...
  const handleDownload = async () => {
    if (selectedDevices.length === 0 || Object.values(selectedKeys).every(k => k.length === 0)) {
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full relative">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          >
            {loading ? 'Generating...' : <><FiDownload className="inline-block mr-2" /> Download Report</>}
          </button>
        </div>
      </div>
    </div>
