    ...
    useEffect(() => {
      const fetchLocations = async () => {
        ...
        for (const param of parameters) {
          if (!param.deviceId || !param.latKey || !param.lonKey) continue;
          try {
            const latRes = await fetch('/api/thingsboard/timeseriesdata', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ token, deviceId: param.deviceId, key: param.latKey, limit: 1 }),
            });
            ...
            const latitude = latData?.[param.latKey]?.[0]?.value ? parseFloat(latData[param.latKey][0].value) : null;
            
            const lonRes = await fetch('/api/thingsboard/timeseriesdata', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ token, deviceId: param.deviceId, key: param.lonKey, limit: 1 }),
            });
            ...
            const longitude = lonData?.[param.lonKey]?.[0]?.value ? parseFloat(lonData[param.lonKey][0].value) : null;
